...
async def get_team() -> SelectorGroupChat:
    ...
    planning_agent = AssistantAgent(
        ...
        まだ回答途中なのに TERMINATE を含めてはいけません。
        """,
    )
    ...
    hotel_agent = AssistantAgent(
        ...
        system_message="""あなたはホテルのエージェントです。ホテルの検索、予約、確認、取り消し、FAQを行います。ただし、今はホテルの検索のみを行います。
        """,
    )
    ...
    airline_agent = AssistantAgent(
        ...
        tools=[search_flight],
        reflect_on_tool_use=True,
        system_message="""あなたは航空会社のエージェントです。航空券の予約、確認、取り消し、FAQを行います。ただし、今は航空券の検索のみを行います。
        """,
    )
