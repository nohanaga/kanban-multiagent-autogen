...
from autogen_agentchat.messages import TextMessage, UserInputRequestedEvent, ToolCallExecutionEvent, ToolCallRequestEvent, AgentEvent, ChatMessage, HandoffMessage
...
# ログの設定
# Selected speaker を表示するためのカスタムハンドラ
class SelectedSpeakerHandler(logging.Handler):
    def emit(self, record):
        msg = self.format(record)
        if "Selected speaker:" in msg:
            selected_speaker = msg.split("Selected speaker: ")[-1].strip()
...
async def get_team() -> SelectorGroupChat:
    ...
    planning_agent = AssistantAgent(
        ...
        handoffs=["HotelAgent", "AirlineAgent"],
        system_message="""
...
- 最終回答が完成したら文の最後に TERMINATE を含めること! ユーザーに回答を求める質問は絶対しないでください。
    ...
    hotel_agent = AssistantAgent(
        ...
        model_client=model_client,
        tools=[search_hotel],
        handoffs=["PlanningAgent", "AirlineAgent"],
        reflect_on_tool_use=True,
        system_message="""あなたはホテルのエージェントです。ホテルの検索、予約、確認、取り消し、FAQを行います。ただし、今はホテルの検索のみを行います。必ず日本語で回答します。
        If necessary to complete the task, handoff the task to a specified agent. Once all tasks have been completed, be sure to handoff to the planner.
        """,
    )
    ...
    airline_agent = AssistantAgent(
        ...
        model_client=model_client,
        tools=[search_flight],
        handoffs=["PlanningAgent", "HotelAgent"],
        reflect_on_tool_use=True,
    ...
    team = Swarm(
        participants=[planning_agent, hotel_agent, airline_agent], termination_condition=combined_termination
    )
